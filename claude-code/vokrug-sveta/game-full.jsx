...
function useGame(count) {
  ...
  function closeTrade() { set({ trade: null }); }
  ...
  function proposeTrade() {
    ...
    if (!E.partnerAcceptsTrade(s, t, 0, partnerIdx)) { set({ toast: `${s.players[partnerIdx].name} отклонил обмен` }); return; }
    set(st => E.applyTrade(st, 0, partnerIdx, t.give, t.get, t.money));
  }
  ...
  function acceptIncomingTrade() {
    ...
    if (!t) return;
    set(st => {
      const next = E.applyTrade(st, t.fromIdx, t.partnerIdx, t.give, t.get, t.money);
  ...
  function acceptIncomingTrade(extra = 0) {
    ...
    after(400, () => {
      const s = ref.current;
      if (s.phase === 'idle' && s.players[s.current]?.id !== 'you') rollDice();
    });
  }
  function declineIncomingTrade() {
    set({ incomingTrade: null });
